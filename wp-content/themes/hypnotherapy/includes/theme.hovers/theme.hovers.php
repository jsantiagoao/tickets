...
if (!function_exists('hypnotherapy_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'hypnotherapy_hovers_theme_setup3', 3 );
	function hypnotherapy_hovers_theme_setup3() {
		// Add 'Buttons hover' option
		hypnotherapy_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'hypnotherapy'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'hypnotherapy') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'hypnotherapy'),
					'slide_left'	=> esc_html__('Slide from Left',	'hypnotherapy'),
					'slide_right'	=> esc_html__('Slide from Right',	'hypnotherapy'),
					'slide_top'		=> esc_html__('Slide from Top',		'hypnotherapy'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'hypnotherapy'),
//					'arrow'			=> esc_html__('Arrow',				'hypnotherapy'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'hypnotherapy'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'hypnotherapy') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'hypnotherapy'),
					'icon'	=> esc_html__('Icon',	'hypnotherapy'),
					'icons'	=> esc_html__('Icons',	'hypnotherapy'),
					'zoom'	=> esc_html__('Zoom',	'hypnotherapy'),
					'fade'	=> esc_html__('Fade',	'hypnotherapy'),
					'slide'	=> esc_html__('Slide',	'hypnotherapy'),
					'pull'	=> esc_html__('Pull',	'hypnotherapy'),
					'border'=> esc_html__('Border',	'hypnotherapy')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('hypnotherapy_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'hypnotherapy_hovers_theme_setup9', 9 );
	function hypnotherapy_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'hypnotherapy_hovers_frontend_scripts', 1010 );
		add_filter( 'hypnotherapy_filter_localize_script','hypnotherapy_hovers_localize_script' );
		add_filter( 'hypnotherapy_filter_merge_scripts',	'hypnotherapy_hovers_merge_scripts' );
		add_filter( 'hypnotherapy_filter_merge_styles',	'hypnotherapy_hovers_merge_styles' );
		add_filter( 'hypnotherapy_filter_get_css', 		'hypnotherapy_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'hypnotherapy_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'hypnotherapy_hovers_frontend_scripts', 1010 );
	function hypnotherapy_hovers_frontend_scripts() {
		if ( hypnotherapy_is_on(hypnotherapy_get_theme_option('debug_mode')) && file_exists(hypnotherapy_get_file_dir('includes/theme.hovers/theme.hovers.js')) )
			wp_enqueue_script( 'hypnotherapy-hovers', hypnotherapy_get_file_url('includes/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( hypnotherapy_is_on(hypnotherapy_get_theme_option('debug_mode')) && file_exists(hypnotherapy_get_file_dir('includes/theme.hovers/theme.hovers.css')) )
			wp_enqueue_style( 'hypnotherapy-hovers',  hypnotherapy_get_file_url('includes/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('hypnotherapy_hovers_merge_scripts')) {
	//Handler of the add_filter( 'hypnotherapy_filter_merge_scripts', 'hypnotherapy_hovers_merge_scripts' );
	function hypnotherapy_hovers_merge_scripts($list) {
...
if (!function_exists('hypnotherapy_hovers_merge_styles')) {
	//Handler of the add_filter( 'hypnotherapy_filter_merge_styles', 'hypnotherapy_hovers_merge_styles' );
	function hypnotherapy_hovers_merge_styles($list) {
...
if (!function_exists('hypnotherapy_hovers_localize_script')) {
	//Handler of the add_filter( 'hypnotherapy_filter_localize_script','hypnotherapy_hovers_localize_script' );
	function hypnotherapy_hovers_localize_script($arr) {
		$arr['button_hover'] = hypnotherapy_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('hypnotherapy_hovers_add_icons') ) {
	function hypnotherapy_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				<a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'hypnotherapy');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'hypnotherapy_hovers_get_css' ) ) {
	//Handler of the add_filter( 'hypnotherapy_filter_get_css', 'hypnotherapy_hovers_get_css', 10, 4 );
	function hypnotherapy_hovers_get_css($css, $colors, $fonts, $scheme='') {
